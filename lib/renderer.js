...
default_rules.heading_open = function (tokens, idx , options, env, slf) {
  ...
  var escaped_content = escapeHtml(tokens[idx].content)
  var h_id = escaped_content;
  if (env.headings[h_id] == undefined) {
    env.headings[h_id] = 1;
  } else {
    h_id += ('-' + String(env.headings[escaped_content]));
    console.log(env);
    env.headings[escaped_content] ++;
    console.log(env);
  }
...
default_rules.code_block = function (tokens, idx, options, env, slf) {
  ...
  return '<div class="highlight"><pre class="code" '  + slf.renderAttrs(token) + '><code>' +
...
default_rules.math_block = function (tokens, idx, options, env, slf) {
  ...
  if (options.mathRender == 'save') {
    ...
    if (options.mathDelimiter == 'brace') {
      html = `\\[${token.content}\\]`
    } else {
      html = `$$${token.content}$$`
    }
...
default_rules.fence = function (tokens, idx, options, env, slf) {
  ...
  highlighted = '<div class="highlight"><pre class="code"><code>' +
  (hljs.getLanguage(langName) ? hljs.highlight(token.content, {language: langName, ignoreIllegals: true}).value : hljs.highlightAuto(token.content).value) +
  '</code></pre></div>';
