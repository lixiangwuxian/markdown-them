...
module.exports = function math_inline(state, silent) {
    ...
    if ((state.md.options.mathInline === 'brace' || state.md.options.mathInline === 'both') && start + 3 <= state.src.length && state.src.slice(start, start + 3) === '\\\\(' && (start === 0 || state.src[start - 1] !== '\\')) {
        // is \(\)
